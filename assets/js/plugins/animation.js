/*
 * Invers Web Animation JS
 *inversweb.com
 * (c) 2025 BM ALAMIN
 * Released under the inversweb license
 */

// Class names for different scroll and animation states
const INVERS_SCROLL_ACTIVATION = "tmp-scroll-trigger";
    INVERS_SCROLL_OFFSCREEN_ACTIVATION = "tmp-scroll-trigger--offscreen",
    INVERS_SCROLL_ZOOM_IN_ACTIVATION = "animate--zoom-in",
    INVERS_SCROLL_CANCEL_ACTIVATION = "tmp-scroll-trigger--cancel";
...
function onIntersection(entries, observer) {
    entries.forEach((entry, index) => {
        if (entry.isIntersecting) {
            ...
            if ($(elementTarget).hasClass(INVERS_SCROLL_OFFSCREEN_ACTIVATION)) {
                $(elementTarget).removeClass(INVERS_SCROLL_OFFSCREEN_ACTIVATION);
            ...
            $(entry.target).addClass(INVERS_SCROLL_OFFSCREEN_ACTIVATION);
            $(entry.target).removeClass(INVERS_SCROLL_CANCEL_ACTIVATION);
        }
    });
}
...
function initializeScrollAnimationTrigger(rootEl = document, isDesignModeEvent = false) {
    const animationTriggerElements = $(rootEl).find(`.${INVERS_SCROLL_ACTIVATION}`);
...
function initializeScrollZoomAnimationTrigger() {
    ...
    const animationTriggerElements = $(`.${INVERS_SCROLL_ZOOM_IN_ACTIVATION}`);
    if (animationTriggerElements.length === 0) return;
